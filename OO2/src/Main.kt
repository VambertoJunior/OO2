...
class Gerente(nome: String, idade: Int, val setor: String) : Funcionario(nome, idade) {
    override fun apresentar() {
        super.apresentar()
        println("Sou gerente do setor $setor.")
    }
}
...
class Desenvolvedor(nome: String, idade: Int, val linguagem: String) : Funcionario(nome, idade) {
    override fun apresentar() {
        super.apresentar()
        println("Trabalho com $linguagem.")
    }
}
...
fun main() {
    val funcionarios = listOf(
        Gerente("Guga", 42, "TI"),
        Desenvolvedor("Ana", 28, "Python"),
        Analista("Lara", 30, "Financeiro")
    )

    funcionarios.forEach { it.apresentar() }
}